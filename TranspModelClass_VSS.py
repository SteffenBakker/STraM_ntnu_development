...
class TranspModel:
    ...
    def __init__(self, instance, one_time_period, scenario, carbon_scenario, fuel_costs):
        ...
        #IMPORT THE DATA
        self.data = TransportSets(self.scenario, self.carbon_scenario, self.fuel_costs)
        ...
        for a in self.data.A_ARCS:
            for p in self.data.P_PRODUCTS:
                for t in self.data.TS_fs:
                    self.model.x_flow[a[0], a[1], a[2], a[3], a[4], p,t].fix(self.data.first_stage_x[a[0], a[1], a[2], a[3], a[4], p,t]/self.data.AVG_DISTANCE[a[0], a[1], a[2], a[4]])
                    #if self.data.first_stage_x[a[0], a[1], a[2], a[3], a[4], p,t]>0 and t==2020 and p=='Dry bulk' and a[0]=='Oslo':
                     #   print('from transpm: ', a,p,t, self.data.first_stage_x[a[0], a[1], a[2], a[3], a[4], p,t])
