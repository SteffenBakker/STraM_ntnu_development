...
with open(r'Data\base_data', 'rb') as data_file:
    ...
    print(len(base_data.E_EDGES))
    print(base_data.E_EDGES)
    ...
    for (i,j,m,r) in base_data.E_EDGES:
        ...
        if i_sea_index < j_sea_index:
            ...
            cur_dest = i
        cur_mode = m
        if ((cur_orig, cur_dest, cur_mode) not in unrouted_edges):  # unobserved edge
            unrouted_edges.append((cur_orig, cur_dest, cur_mode)) # add to list
        else:
            break # don't continue if we've already seen this edge
